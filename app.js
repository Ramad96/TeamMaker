...
function initGame() {
    ...
    for (let i = 0; i < num; i++) {
        const s = i * perCourt;
        courts.push({ teamA: players.slice(s, s + teamSize), teamB: players.slice(s + teamSize, s + perCourt), winner: null });
    }
...
function confirmRotation() {
    ...
    historyData.unshift({ teamA: [...court.teamA], teamB: [...court.teamB], score: `${sA}-${sB}`, winnerKey: key });
    const loserKey = key === 'teamA' ? 'teamB' : 'teamA';
    if (waitingQueue.length >= teamSize) { waitingQueue.push(...court[loserKey]); court[loserKey] = waitingQueue.splice(0, teamSize); }
    else { let s = [...court.teamA, ...court.teamB].sort(() => Math.random() - 0.5); court.teamA = s.slice(0, teamSize); court.teamB = s.slice(teamSize); }
    court.winner = null; pendingRotation = null;
...
function render() {
            ...
            <div class="space-y-2">
                <div onclick="handleSlotClick('court', 'teamA', ${i}, 0)" class="flex flex-col gap-2 p-4 bg-slate-900 rounded-[2rem] border-2 transition-all ${c.winner === 'teamA' ? 'winner-glow' : 'border-slate-800'}">
                    ...
                    <div class="flex justify-center gap-2">
                        ${Array.from({length: teamSize}).map((_, si) => `<div class="player-slot px-4 py-2 rounded-xl font-bold text-sm min-w-[80px] text-center ${c.teamA[si] ? 'slot-filled' : 'slot-empty'} ${editMode && selectedFromPool !== null && !c.teamA[si] ? 'draft-active' : ''}">${c.teamA[si] || '...'}</div>`).join('')}
                    </div>
                </div>
                ...
                <div onclick="handleSlotClick('court', 'teamB', ${i}, 0)" class="flex flex-col gap-2 p-4 bg-slate-900 rounded-[2rem] border-2 transition-all ${c.winner === 'teamB' ? 'winner-glow' : 'border-slate-800'}">
                    ...
                    <div class="flex justify-center gap-2">
                        ${Array.from({length: teamSize}).map((_, si) => `<div class="player-slot px-4 py-2 rounded-xl font-bold text-sm min-w-[80px] text-center ${c.teamB[si] ? 'slot-filled' : 'slot-empty'} ${editMode && selectedFromPool !== null && !c.teamB[si] ? 'draft-active' : ''}">${c.teamB[si] || '...'}</div>`).join('')}
                    </div>
                </div>
            </div>`;
    ...
    document.getElementById('waitingList').innerHTML = waitingQueue.map((p, i) => `<div onclick="handleSlotClick('waiting', null, null, ${i})" class="px-4 py-2 bg-slate-800 rounded-xl font-bold text-sm slot-filled">${p}</div>`).join('');
